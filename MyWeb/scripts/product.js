...
$(document).ready(function () {
    var url_found = checkUrl(); if (!url_found) { if (typeof productHasAttributes != 'undefined' && productHasAttributes) { findCombination(); } else { refreshProductImages(0); } }
    if (typeof (jqZoomEnabled) !== 'undefined' && jqZoomEnabled) {
        if ($('#thumbs_list .shown img').length) { var new_src = $('#thumbs_list .shown img').attr('src').replace('cart_', 'large_'); if ($('.jqzoom img').attr('src') != new_src) { $('.jqzoom img').attr('src', new_src).parent().attr('href', new_src); } }
    ...
    var domData = $("#quantityDiscount table tbody tr").not(":hidden"); var nbProduct = $('#quantity_wanted').val(); var newPrice = false; domData.each(function (i) {
        var dataDiscountQuantity = parseInt($(this).attr('data-discount-quantity')); var dataDiscountNextQuantity = -1; var nextQtDiscount = $(domData[i + 1]); if (nextQtDiscount.length) { dataDiscountNextQuantity = parseInt(nextQtDiscount.attr('data-discount-quantity')); }
        if ((dataDiscountNextQuantity != -1 && nbProduct >= dataDiscountQuantity && nbProduct < dataDiscountNextQuantity) || (dataDiscountNextQuantity == -1 && nbProduct >= dataDiscountQuantity)) { newPrice = $(this).attr('data-real-discount-value'); return false; }
    }); return newPrice;
}
$(window).resize(function () { $('#thumbs_list').trigger('goto', 0); serialScrollFixLock('', '', '', '', 0); }); $(window).bind('hashchange', function () { checkUrl(); findCombination(); }); if (!jqZoomEnabled) { $(document).on('mouseover', '#views_block li a', function () { displayImage($(this)); }); }
...
if (typeof (contentOnly) != 'undefined' && contentOnly) {
    $(document).on('click', '.fancybox', function (e) { e.preventDefault(); if (typeof (jqZoomEnabled) != 'undefined' && jqZoomEnabled) { displayImage($(this)) } }); $(document).on('click', '#image-block', function (e) {
        e.preventDefault(); var productUrl = window.document.location.href + ''; var data = productUrl.replace(/[\?|&]content_only=1/, ''); if (window.parent.page_name == 'search') { data += ((data.indexOf('?') < 0) ? '?' : '&') + 'HTTP_REFERER=' + encodeURIComponent(window.parent.document.location.href); }
        window.parent.document.location.href = data; return;
    });
}
$(document).on('click', '.product_quantity_up', function (e) {
    e.preventDefault(); fieldName = $(this).data('field-qty'); var currentVal = parseInt($('input[name=' + fieldName + ']').val()); if (!allowBuyWhenOutOfStock && quantityAvailable > 0) { quantityAvailableT = quantityAvailable; } else { quantityAvailableT = 100000000; }
    ...
    e.preventDefault(); fieldName = $(this).data('field-qty'); var currentVal = parseInt($('input[name=' + fieldName + ']').val()); if (!isNaN(currentVal) && currentVal > 1) { $('input[name=' + fieldName + ']').val(currentVal - 1).trigger('keyup'); } else { $('input[name=' + fieldName + ']').val(1); }
    $('#quantity_wanted').change();
}); if (typeof minimalQuantity != 'undefined' && minimalQuantity) { checkMinimalQuantity(); $(document).on('keyup', 'input[name=qty]', function (e) { checkMinimalQuantity(minimalQuantity); }); }
...
function findCombination() {
    $('#minimal_quantity_wanted_p').fadeOut(); if (typeof $('#minimal_quantity_label').text() === 'undefined' || $('#minimal_quantity_label').html() > 1) { $('#quantity_wanted').val(1); }
    ...
    $('#attributes select, #attributes input[type=hidden], ' + radio_inputs).each(function () { choice.push(parseInt($(this).val())); }); if (typeof combinationsHashSet !== 'undefined') {
        var combination = combinationsHashSet[choice.sort().join('-')]; if (combination) {
            if (combination['minimal_quantity'] > 1) { $('#minimal_quantity_label').html(combination['minimal_quantity']); $('#minimal_quantity_wanted_p').fadeIn(); $('#quantity_wanted').val(combination['minimal_quantity']); $('#quantity_wanted').bind('keyup', function () { checkMinimalQuantity(combination['minimal_quantity']); }); }
...
function updateDisplay() {
    var productPriceDisplay = productPrice; var productPriceWithoutReductionDisplay = productPriceWithoutReduction; if (!selectedCombination['unavailable'] && quantityAvailable > 0 && productAvailableForOrder == 1) {
        $('#quantity_wanted_p:hidden').show('slow'); $('#add_to_cart:hidden').fadeIn(600); $('#oosHook').hide(); $('#availability_date').fadeOut(); if (stock_management && availableNowValue != '') { $('#availability_value').removeClass('warning_inline').text(availableNowValue).show(); $('#availability_statut:hidden').show('slow'); } else { $('#availability_statut:visible').hide('slow'); }
        if (!allowBuyWhenOutOfStock) { if (quantityAvailable <= maxQuantityToAllowDisplayOfLastQuantityMessage) { $('#last_quantities').show('slow'); } else { $('#last_quantities').hide('slow'); } }
        if (quantitiesDisplayAllowed) { $('#pQuantityAvailable:hidden').show('slow'); $('#quantityAvailable').text(quantityAvailable); if (quantityAvailable < 2) { $('#quantityAvailableTxt').show(); $('#quantityAvailableTxtMultiple').hide(); } else { $('#quantityAvailableTxt').hide(); $('#quantityAvailableTxtMultiple').show(); } }
    } else {
        if (productAvailableForOrder == 1) { $('#oosHook').show(); if ($('#oosHook').length > 0 && function_exists('oosHookJsCode')) { oosHookJsCode(); } }
        $('#last_quantities:visible').hide('slow'); $('#pQuantityAvailable:visible').hide('slow'); if (!allowBuyWhenOutOfStock) { $('#quantity_wanted_p:visible').hide('slow'); }
        ...
        if (allowBuyWhenOutOfStock && !selectedCombination['unavailable'] && productAvailableForOrder) { $('#add_to_cart:hidden').fadeIn(600); if (stock_management && availableLaterValue != '') { $('#availability_value').addClass('label-warning').text(availableLaterValue).show('slow'); $('#availability_statut:hidden').show('slow'); } else { $('#availability_statut:visible').hide('slow'); } } else { $('#add_to_cart:visible').fadeOut(600); if (stock_management == 1 && productAvailableForOrder) { $('#availability_statut:hidden').show('slow'); } }
...
function updatePrice() {
    ...
    if (combination.specific_price && combination.specific_price.reduction > 0) {
        if (combination.specific_price.reduction_type == 'amount') {
            if (typeof combination.specific_price.reduction_tax === 'undefined' || (typeof combination.specific_price.reduction_tax !== 'undefined' && combination.specific_price.reduction_tax === '1')) {
                var reduction = combination.specific_price.reduction; if (typeof specific_currency !== 'undefined' && specific_currency && parseInt(combination.specific_price.id_currency) && combination.specific_price.id_currency != currency.id) { reduction = reduction / currencyRate; } else if (!specific_currency) { reduction = reduction * currencyRate; }
    ...
    $('#reduction_percent').hide(); $('#reduction_amount').hide(); $('#our_price_display').removeClass('new-price'); $('#old_price, #old_price_display, #old_price_display_taxes').hide(); $('.price-ecotax').hide(); $('.unit-price').hide(); if (priceWithDiscountsDisplay > 0) { $('#our_price_display').text(formatCurrency(priceWithDiscountsDisplay, currencyFormat, currencySign, currencyBlank)).trigger('change'); } else { $('#our_price_display').text(formatCurrency(0, currencyFormat, currencySign, currencyBlank)).trigger('change'); }
    if (priceWithDiscountsDisplay.toFixed(2) != basePriceDisplay.toFixed(2)) {
        $('#old_price_display span.price').text(formatCurrency(basePriceDisplay, currencyFormat, currencySign, currencyBlank)); $('#old_price, #old_price_display, #old_price_display_taxes').removeClass('hidden').show(); if (priceWithDiscountsWithoutTax != priceWithGroupReductionWithoutTax) {
            if (combination.specific_price.reduction_type == 'amount') { $('#reduction_amount_display').html('-' + formatCurrency(discountValue, currencyFormat, currencySign, currencyBlank)); $('#reduction_amount').show(); $('#our_price_display').addClass('new-price'); } else {
                var toFix = 2; if ((parseFloat(discountPercentage).toFixed(2) - parseFloat(discountPercentage).toFixed(0)) == 0) { toFix = 0; }
    ...
    if (default_eco_tax) {
        ecotax = default_eco_tax; if (combination.ecotax) { ecotax = +combination.ecotax; }
...
function serialScrollFixLock(event, targeted, scrolled, items, position) {
    if ($('body').find('#image-block').parent().innerWidth() > 485) { serialScrollNbImagesDisplayed = 5; } else if (($('body').find('#image-block').parent().innerWidth() > 380) && ($('body').find('#image-block').parent().innerWidth() <= 485)) {
        serialScrollNbImagesDisplayed = 4; if (productColumns == 2)
            serialScrollNbImagesDisplayed = 4;
    } else { serialScrollNbImagesDisplayed = 3; }
    if ($('body').find('#image-block').parent().innerWidth() > 580) { serialScrollNbImagesDisplayed = 5; }
    if ($('body').find('#image-block').parent().innerWidth() > 799) { serialScrollNbImagesDisplayed = 7; }
    if (typeof (contentOnly) != 'undefined' && contentOnly)
        serialScrollNbImagesDisplayed = 5; serialScrollNbImages = $('#thumbs_list li:visible').length; var leftArrow = position == 0 ? true : false; var rightArrow = position + serialScrollNbImagesDisplayed >= serialScrollNbImages ? true : false; $('#view_scroll_left').css('cursor', leftArrow ? 'default' : 'pointer').fadeTo(0, leftArrow ? 0 : 1).css('display', leftArrow ? 'none' : 'block'); $('#view_scroll_right').css('cursor', rightArrow ? 'default' : 'pointer').fadeTo(0, rightArrow ? 0 : 1).css('display', rightArrow ? 'none' : 'block'); return true;
}
function refreshProductImages(id_product_attribute) {
    $('#thumbs_list_frame').scrollTo('li:eq(0)', 500, { axis: 'y' }); id_product_attribute = parseInt(id_product_attribute); if (id_product_attribute > 0 && typeof (combinationImages) !== 'undefined' && typeof (combinationImages[id_product_attribute]) !== 'undefined') { $('#thumbs_list li').hide(); $('#thumbs_list').trigger('goto', 0); for (var i = 0; i < combinationImages[id_product_attribute].length; i++) { if (typeof (jqZoomEnabled) !== 'undefined' && jqZoomEnabled) { $('#thumbnail_' + parseInt(combinationImages[id_product_attribute][i])).show().children('a.shown').trigger('click'); } else { $('#thumbnail_' + parseInt(combinationImages[id_product_attribute][i])).show(); } } } else {
        $('#thumbs_list li').show(); var choice = []; var radio_inputs = parseInt($('#attributes .checked > input[type=radio]').length); if (radio_inputs) { radio_inputs = '#attributes .checked > input[type=radio]'; } else { radio_inputs = '#attributes input[type=radio]:checked'; }
    ...
    var thumb_height = $('#thumbs_list_frame >li').outerHeight() + parseInt($('#thumbs_list_frame >li').css('marginTop')); $('#thumbs_list_frame').height((parseInt((thumb_height) * $('#thumbs_list_frame >li').length)) + 'px'); $('#thumbs_list').trigger('goto', 0); serialScrollFixLock('', '', '', '', 0);
}
...
$(document).ready(galeryReload); $(window).resize(refreshProductImages); function saveCustomization() { $('#quantityBackup').val($('#quantity_wanted').val()); $('#customizationForm').submit(); }
...
function getProductAttribute() {
    request = ''; var tab_attributes = []; var radio_inputs = parseInt($('#attributes .checked > input[type=radio]').length); if (radio_inputs) { radio_inputs = '#attributes .checked > input[type=radio]'; } else { radio_inputs = '#attributes input[type=radio]:checked'; }
    ...
    if ($('#customizationForm').length) {
        customAction = $('#customizationForm').attr('action'); if (customAction.indexOf('#') != -1) { customAction = customAction.substring(0, customAction.indexOf('#')); }
...
function checkUrl() {
    if (original_url != window.location || first_url_check) {
        first_url_check = false; var url = window.location + ''; if (url.indexOf('#/') != -1) {
            params = url.substring(url.indexOf('#') + 1, url.length); tabParams = params.split('/'); tabValues = []; if (tabParams[0] == '') { tabParams.shift(); }
            ...
            if (count) {
                if (firstTime) { firstTime = false; findCombination(); }
...
$(document).ready(function (e) {
    if (productColumns != 'undefined') {
        if ($(document).width() >= 768) {
            ...
            maxSlides = 6;
        } else { minSlides = 3; maxSlides = 3; }
    } else { minSlides = 2 }
    if ($('#bxslider li').length && !!$.prototype.bxSlider) { products_slider = $('#bxslider').bxSlider({ minSlides: minSlides, maxSlides: maxSlides, slideWidth: 178, slideMargin: 20, pager: false, nextText: '', prevText: '', moveSlides: 1, infiniteLoop: false, hideControlOnEnd: true }); }
    if ($('#bxslider').length) {
        $(window).on('resize load', function () {
            if ($(document).width() <= 767) { if (($(document).width() <= 500)) { products_slider.reloadSlider({ minSlides: 2, maxSlides: 2, slideWidth: 178, slideMargin: 20, pager: false, nextText: '', prevText: '', moveSlides: 1, infiniteLoop: false, hideControlOnEnd: true }); } else { products_slider.reloadSlider({ minSlides: 3, maxSlides: 3, slideWidth: 178, slideMargin: 20, pager: false, nextText: '', prevText: '', moveSlides: 1, infiniteLoop: false, hideControlOnEnd: true }); } } else if (($(document).width() <= 1199) && ($(document).width() >= 768)) {
                if (($(document).width() <= 991)) {
                    if (productColumns != 'undefined') { if (productColumns == 1) { minSlides = 4; } else if (productColumns == 2) { minSlides = 3; } else { minSlides = 2; } } else { minSlides = 2; }
                    products_slider.reloadSlider({ minSlides: minSlides, maxSlides: 4, slideWidth: 178, slideMargin: 20, pager: false, nextText: '', prevText: '', moveSlides: 1, infiniteLoop: false, hideControlOnEnd: true });
                } else {
                    if (productColumns != 'undefined') { if (productColumns == 1) { minSlides = 5; } else if (productColumns == 2) { minSlides = 4; } else { minSlides = 3; } } else { minSlides = 2; }
                    products_slider.reloadSlider({ minSlides: minSlides, maxSlides: 5, slideWidth: 178, slideMargin: 20, pager: false, nextText: '', prevText: '', moveSlides: 1, infiniteLoop: false, hideControlOnEnd: true });
                }
            } else if ($(document).width() >= 1200) {
                if (productColumns != 'undefined') { if (productColumns == 1) { minSlides = 6; } else if (productColumns == 2) { minSlides = 5; } else { minSlides = 3; } } else { minSlides = 2 }
                products_slider.reloadSlider({ minSlides: minSlides, maxSlides: 6, slideWidth: 178, slideMargin: 20, pager: false, nextText: '', prevText: '', moveSlides: 1, infiniteLoop: false, hideControlOnEnd: true })
            }
        })
    }
    if (!$('#bxslider li').length) { $('.accessories-block').parent().remove(); }
}); $(document).on('click', '.product-info-tabs li a', function () { var contentElement = $(this).attr('href'); if ($(document).width() < 768) { $.scrollTo(contentElement, 1200); } })